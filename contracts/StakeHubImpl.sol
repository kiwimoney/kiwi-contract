...
contract StakeHubImpl is Context, Initializable, ReentrancyGuard {
    ...
    address public constant validatorContractAddr = 0x80d1769ac6fee59BE5AAC1952a90270bbd2Ceb2F;

    mapping(address => StakeAgentImpl) public userStakeAgentMap;

    uint256 public stakeFeeMolecular;
    uint256 constant public stakeFeeDenominator = 10000;
    ...
    function setAdmin(address newAdmin_) external {
        ...
        require(newAdmin_ != address(0x00), "new admin must not be zero address.");
        admin = newAdmin_;

        emit NewAdmin(newAdmin_);
    }

    function stake(uint256 pid, uint256 amount) nonReentrant whenNotPaused external payable returns (bool) {
        ...
        stakeAgent.stake{value: amount}(pid);
    ...
    function unstake(uint256 pid, uint256 amount) nonReentrant whenNotPaused external returns (bool) {
        StakeAgentImpl stakeAgent = userStakeAgentMap[msg.sender];
        require(address(stakeAgent)!=address(0x0),"user never stake");

        IERC20(rHT).safeTransferFrom(msg.sender, address(this), amount);
        IMintBurnToken(rHT).burn(amount);

        stakeAgent.unstake(pid, amount);

        emit LogStake(msg.sender, amount);
        return true;
    }

    function claimPendingUnstake(uint256 pid) nonReentrant whenNotPaused external returns (bool) {
        StakeAgentImpl stakeAgent = userStakeAgentMap[msg.sender];
        require(address(stakeAgent)!=address(0x0),"user never stake");
        stakeAgent.claimPendingUnstake(pid);
        return true;
    }

    function claimStakeProfit(uint256 pid) nonReentrant whenNotPaused external returns (bool) {
        StakeAgentImpl stakeAgent = userStakeAgentMap[msg.sender];
        require(address(stakeAgent)!=address(0x0),"user never stake");
        stakeAgent.claimStakeProfit(pid);
        return true;
    }

    function pendingUnstakeClaimTime(uint256 pid, address staker) external view returns (uint256) {
